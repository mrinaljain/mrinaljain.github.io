...
async function main() {
  ...
  if (!mongoUri) {
    throw new Error("MONGODB_URI is required. Add it to .env.local or your shell env.");
  }
  ...
  const collection = mongoose.connection.collection("experiences");
  let insertedOrUpdated = 0;

  for (const item of experienceSeedData) {
    const slug = toSlug(`${item.company}-${item.designation}`) || toSlug(item.company) || "experience";
    ...
    const payload = {
      ...
      isCurrent: typeof item.isCurrent === "boolean" ? item.isCurrent : !item.endDate,
      summary: item.summary,
      projects: item.projects,
      technologyKeys: item.technologyKeys,
      order: item.order,
      accentColor: item.accentColor,
      status: item.status || "published",
      isFeatured: typeof item.isFeatured === "boolean" ? item.isFeatured : true,
      updatedAt: new Date(),
    };
    ...
    console.log(`Upserted: ${item.company} (${slug})`);
  }

  console.log(`Done. Processed ${experienceSeedData.length} records, changed ${insertedOrUpdated}.`);
  await mongoose.disconnect();
}

main().catch(async (error) => {
  console.error("migrate:experiences failed", error);
  try {
    await mongoose.disconnect();
  } catch {
    // no-op
